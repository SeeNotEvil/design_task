...
1) 3D Модели с текстурами, готовые к импорту в игровой движок unreal engine 5.   
2) Ряд скриншотов с отрандеренными 3D моделями и текстурами, минимум 5-6 штук, с разных ракурсов. Обязательно фронтальное изображение и перспектива.        
3) Оригинальные 3D модели в vbx, с текстурами.     
4) Скриншоты с основными размерами 3D моделей (смотри раздел размеры).   

#### Требования к 3D моделям   
1) Ограничения на полигоны нет.   
2) High poly модели приветствуется.   
...
4 - Обрамление - ширина 1.1 см по краям. Активная область + обрамление = 20.3 см + 1.1 см + 1.1 см = 22.5 см шириной.     
5 - Наконечники - размеры подбираются индивидуально, около 1.5 см ширина и диаметр 6.5 см (можно делать больше). Планка + Наконечники = 26.5 см + 1.5 см + 1.5 см = 29.5 см шириной.         
...
**Тело свитка крепится к середине планки (это нормально - свиток декоративный будет), спереди активная зона занимает 27.1 см ровно (сколько см занимает сзади не важно), зазора между планкой и активной зоной быть не должно, она должна быть полностью видна с фронтальной перспективы**   
...
Пергаменты нужны, **как пустые**, так и **с текстом (текст рыба)** с разными чернилами.     
